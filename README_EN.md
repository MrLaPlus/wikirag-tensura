...
Content derived from the [Tensura Wiki](https://tensura.fandom.com) is distributed under the **Creative Commons Attribution-ShareAlike 3.0 Unported (CC BY-SA 3.0)** license. Generated answers include attribution automatically. See [`NOTICE.md`](NOTICE.md) for attribution details.

The embedding model is [gpahal/bge-m3-onnx-int8](https://huggingface.co/gpahal/bge-m3-onnx-int8), whose model card identifies an MIT license. The base model is [BAAI/bge-m3](https://huggingface.co/BAAI/bge-m3). Preserve the applicable model license notices when downloading or distributing model files.
